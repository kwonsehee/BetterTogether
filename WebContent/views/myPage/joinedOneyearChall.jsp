      ...
      <% } else { %>
       ...
       <% } %>
        ...
        <section id="page_css">
			...
			<button onclick="location.href='<%= request.getContextPath() %>/joined/oneyear?currentPage=1'"> &lt;&lt;</button>	
			...
			<!-- 참여했던 챌린지 기본 메인 페이지 -->
			<% } else {%>			
	        <button onclick="location.href='<%= request.getContextPath() %>/joined/oneyear?currentPage=<%= pi.getCurrentPage() - 1%>'"> &lt; </button>
			...
			<% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++) {%>
				...
				<% } else {%>
				<button onclick="location.href='<%= request.getContextPath() %>/joined/oneyear?currentPage=<%=p %>'"> <%= p %> </button>
					<%-- <% if(ds.getThreeMonths() <=3) { %>
			...
			<% } %>
             ...
             <!-- 맨 끝으로 (>>) -->
			 <button onclick="location.href='<%= request.getContextPath() %>/joined/oneyear?currentPage=<%= pi.getMaxPage() %>'"> &gt;&gt; </button>
           	<%--  <% if(ds.getThreeMonths() <=3) { %>
